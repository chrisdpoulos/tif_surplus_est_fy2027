...
def load_data():
    ...
    try:
        ...
        df = df.dropna(subset=["unallocated_funds_2026"])
...
# Add a sidebar with methodology
    ...
    st.sidebar.markdown("""
    This report provides estimates of TIF surplus estimates for the City of Chicago for fiscal year 2027. Below is a summary of each estimate methodology.
    
    1. **Unallocated Funds**: The total unallocated funds as of December 31, 2025 from the [City of Chicago's "10-Year TIF Projections: 2025 Year-End Report: Discover the Future of Chicago's Development" (last updated: 3/11/2026)](https://www.chicago.gov/content/dam/city/depts/obm/supp_info/TIF/TIF%20Projection%202025%20Yearend%20Report.pdf). Unallocated Funds are "the amount left over after taking into account the funds carried over from the fund balance, the revenue anticipated to be generated within the TIF District, and the amounts set aside for current obligations and proposed projects. The Unallocated Fund amount reflects the anticipated funding that will remain in the TIF District on December 31st of the year in question" (p. 3, How to Read This Report). This method assumes that all unallocated funds could be declared as surplus. 

    2. **City Surplus Method**: [PHASED OUT BECAUSE CITY DOES NOT APPLY THIS METHOD]. Previously, the City of Chicago's Office of Management and Budget (OMB) calculated the surplus for each TIF district based on the following formula: 25\\% of unallocated cash between \\$750k and \\$1.5m, 75\\% of unallocated cash between \\$1.5m and \\$2.5m, and 100\\% of unallocated cash over \\$2.5m ([Chicago OMB, p. 14](https://igchicago.org/wp-content/uploads/2022/01/OIG-Audit-of-the-Citys-Compliance-with-the-TIF-Sunshine-Ordinance-and-TIF-Surplus-Executive-Order.pdf)). We apply this calculation to the City's Unallocated Fund Year End 2025.

    The data for the following methods are from TIF annual audits will be available after July 1st. These audits are required to be submitted to the Illinois Comptroller no later than 180 days after the close of the municipal fiscal year (Chicago's fiscal year ends December 31st), as mandated by 65 ILCS 5/11-74.6-22(d). Ignore the specific years until the data is available and the estimates are completed.
    
    **TIF audit method**: This methodology uses Illinois Comptroller Annual TIF Audit Reports (required by state law ) to collect data on fund balance, debt obligation, and public investment that is undertaken, as well as Cook County Clerk TIF Agency Distribution Reports to collect data on tax rates with in TIF, TIF incremental EAV, and TIF revenue. TIF surplus is calculated as following: fund balance plus projected revenue minus debt obligation minus public investment. To estimate future revenue we project out EAV using 3 methods and multiply the estimated EAV by the current TIF tax rates. Note: There are multiple tax rates within each TIF. TIFs are segmented into multiple tax codes. We use the average tax rate in the TIF as the TIF district tax rate.
    
    4. **TIF audit method 1**: Projected EAV growth is calculated as the average of year-to-year growth rates between tax year 2006 and tax year 2023. We include this average in the estimation for the the following year. 
        
    5. **TIF audit method 2**: Projects EAV growth using polynomial regression, which takes into account the non-linearity of EAV growth rates.
    
    6. **TIF audit method 3**: As with method 2, this method attempts to account for non-linearity. It takes the average of the last 4 years while privileging assessment years, which tend to see spikes in EAV growth. We weighted the 2024 estimation for the subsequent EAV projection as it was also an assesment year.
    
    """)

# Show City Surplus Totals

    st.header("Total TIF Surplus Estimates for FY 2027")

    unallocated_tot = df['unallocated_funds_2026'].sum()
#     surplus_city_tot = df['surplus_2027'].sum()
#     ctu_method1_tot = df['full_surplus_avg_method_27'].sum()
#     ctu_method2_tot = df['full_surplus_poly_method_27'].sum()
#     ctu_method3_tot = df['full_surplus_weighted_method_25'].sum()
    
     # Calculate portion alloted to CPS and City
     # First calculate % of total property tax rate (https://www.cps.edu/globalassets/cps-global-media/banner-images/annual-financial-report/fy25-acfr-final.pdf, pp. 143)
...
#     ctu_method3_tot_chi = ctu_method3_tot * chi
    ...
    # })
    table = pd.DataFrame({
        "Surplus Method": ["Unallocated funds"],
        "Surplus Amount": [unallocated_tot],
        "CPS Surplus Revenue": [unallocated_tot_cps],
        "City of Chicago Surplus Revenue": [unallocated_tot_chi]
    })
...
# Create a filter to select by TIF district default
    ...
    selected_district = st.selectbox("Select TIF District", tif_districts,index=0)
...
#     ctu_method3_chi = ctu_method3 * chi
    ...
    table = pd.DataFrame({
        "Surplus Method": ["Unallocated funds"],
        "Surplus Amount": [unallocated],
        "CPS Surplus Revenue": [unallocated_cps],
        "City of Chicago Surplus Revenue": [unallocated_chi]
    })
...
# Filter for top 5 districts
    ...
    # Filter for districts with the top 5 largest unallocated_funds_2026 amounts
    top5 = df.nlargest(5, 'unallocated_funds_2026')
    
    # Filter for needed columns
    filter_cols = ['tif_name_comptroller_report','unallocated_funds_2026']
    ...
    top5.rename(columns={
        ...
        'unallocated_funds_2026': 'Unallocated Funds'
    }, inplace=True)
    ...
    with col2:
        ...
        numeric_cols = ['Unallocated Funds']
...
#     # 2) Group by ward_id and sum the values for each ward
    ...
    df3['unallocated_funds_rev_2026'] = df3['unallocated_funds_2026'] * cps
...
#     df3['full_surplus_weighted_method_rev_25'] = df3['full_surplus_weighted_method_25'] * cps
    ...
    df4 = df3.groupby('ward_id').agg({
        'unallocated_funds_2026': 'sum',        
        'unallocated_funds_rev_2026': 'sum'
    }).reset_index()
...
#     }).reset_index()
    ...
    df4['unallocated_funds_2026'] = df4['unallocated_funds_2026'].apply(lambda x: f"${x:,.0f}")
...
#     df4['full_surplus_weighted_method_25'] = df4['full_surplus_weighted_method_25'].apply(lambda x: f"${x:,.0f}")    
    df4['unallocated_funds_rev_2026'] = df4['unallocated_funds_rev_2026'].apply(lambda x: f"${x:,.0f}")
...
#     df4['full_surplus_weighted_method_rev_25'] = df4['full_surplus_weighted_method_rev_25'].apply(lambda x: f"${x:,.0f}")
    ...
    df4.rename(columns={
        'ward_id': 'Ward ID',
        'unallocated_funds_2026': 'Surplus Est: Unallocated Funds',
        'unallocated_funds_rev_2026': 'CPS Revenue Est: Unallocated Funds',
    }, inplace=True)
...
#     }, inplace=True)
    ...
    selected_wards = st.selectbox("Select a ward", wards,index=32)

    filtered_df = df3[(df3['ward_id'] == selected_wards)]

    # Filter for needed columns
    filter_cols = ['tif_name_comptroller_report', 'tif_coverage','unallocated_funds_2026']
...
# Download the data.csv file
    ...
    df['unallocated_funds_2026'] = df['unallocated_funds_2026'].apply(lambda x: f"${x:,.0f}")
#    df['surplus_2026'] = df['surplus_2026'].apply(lambda x: f"${x:,.0f}")
#     df['full_surplus_avg_method_26'] = df['full_surplus_avg_method_26'].apply(lambda x: f"${x:,.0f}")
#     df['full_surplus_poly_method_26'] = df['full_surplus_poly_method_26'].apply(lambda x: f"${x:,.0f}")
#     df['full_surplus_weighted_method_26'] = df['full_surplus_weighted_method_26'].apply(lambda x: f"${x:,.0f}")
    ...
    df = df[['TIF District', 'TIF Number', 'Unallocated Funds']]
